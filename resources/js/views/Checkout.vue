<template>
    <div class="flex justify-center">
        <div v-if="!$store.state.thankYouPageVisible" class="mw7 ph3 w-100">
                        ...
                        <div class="flex flex-column f3 ">
                            ...
                            <div v-for="item in $store.state.cart" class="flex relative mb3">
                                ...
                                <div class="w-40 nested-img dn db-ns">
                                    <img :src="(item.image) ? '/storage/' + item.image : item.poster_image" style="max-height:150px; width:auto" :alt="item.name">
                                </div>
        ...
        <div v-else class="mw7 ph3 w-100 flex justify-center">
                ...
                axios.post('/api/orders/send', str)
                    .then((response) => {
                        ...
                        if(json.hasOwnProperty('error')){
                            ...
                            if(json.error == 34){
                                this.error = 'Введите действительный номер телефона';
                            }else if(json.error == 37){
                                this.error = 'Перепроверьте номер телефона';
                            }else{
                                this.error = json.message;
                            }

                        }else{
                            this.$store.commit('showThankYouPage');
                            window.scrollTo({
                                top: 0,
                                behavior: "smooth"
                            });
