...
class CategoryRepository extends BaseRepository implements CategoryContract
{
    ...
    {
        $spot = DB::table('spots')->where('slug', $slug)->first();
        if(!$spot){
            abort(404);
        }
        return $this->all()->where('spot_id', '=', $spot->id)->where('hidden', '0');
    }
...
>>>>>>> cd085b937e83182edb6f3507e214e756ea364aeb
            ->first();
    }

    public function findBySlugAndSpot($slug, $spotSlug)
    {
        $spot_id = DB::table('spots')->where('slug', $spotSlug)->get('id')->first();
